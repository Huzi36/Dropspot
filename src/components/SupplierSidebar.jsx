...
import {
    ...
    Truck, MessageCircle, Settings, LogOut, ChevronLeft, ChevronRight,
} from 'lucide-react'
...
export default function SupplierSidebar({ open, setOpen }) {
    ...
    const NAV = [
        { icon: LayoutDashboard, label: 'Dashboard', path: '/supplier-console/dashboard' },
        { icon: Package, label: 'My Products', path: '/supplier-console/products' },
        { icon: ShoppingCart, label: 'Orders', path: '/supplier-console/orders' },
        { icon: Truck, label: 'Shipping', path: '/supplier-console/shipping' },
        { icon: MessageCircle, label: 'Q&A', path: '/supplier-console/qa', badge: unansweredCount },
        { icon: User, label: 'Account', path: '/supplier-console/account' },
        { icon: Settings, label: 'Settings', path: '/supplier-console/settings' },
    ]
    ...
    return (
        <aside className={`${open ? 'w-64' : 'w-20'} transition-all duration-300 bg-[#143D59] min-h-screen flex flex-col fixed top-0 left-0 z-10`}>
                ...
                {NAV.map(item => {
                    ...
                    return (
                        <Link key={item.path} to={item.path}
                            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group ${active ? 'bg-white/15 text-white' : 'text-white/50 hover:text-white hover:bg-white/10'}`}>
            ...
            <div className="p-3 border-t border-white/10">
                ...
                <button onClick={async () => { await signOut(); window.location.href = '/supplier-console' }}
