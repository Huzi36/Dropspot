        ...
        <div className="min-h-screen bg-[#F7F8FA] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <main className={`flex-1 ${sidebarOpen ? 'ml-64' : 'ml-20'} transition-all duration-300 p-8`}>
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">
                            ...
                            {unansweredCount > 0 && (
                                <span className="ml-2 bg-red-100 text-red-600 text-xs font-bold px-2 py-0.5 rounded-full">{unansweredCount} unanswered</span>
                            )}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 mb-6">
                    {[{ key: 'unanswered', label: 'Unanswered' }, { key: 'answered', label: 'Answered' }, { key: 'all', label: 'All Questions' }].map(tab => (
                        <button key={tab.key} onClick={() => setFilter(tab.key)}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${filter === tab.key ? 'bg-[#143D59] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                            {tab.label}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                        ...
                        <p className="text-gray-500 font-medium">
                            {filter === 'unanswered' ? "No unanswered questions — you're all caught up! 🎉" : 'No questions yet'}
                        </p>
                    </div>
                ) : (
                    <div className="space-y-4">
                                ...
                                <div className="p-6">
                                    <div className="flex items-start gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-400 mt-1">— {q.name}</p>
                                        </div>
                                    </div>
                                    {q.answer ? (
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <button onClick={() => submitAnswer(q.id)} disabled={submitting[q.id] || !answers[q.id]?.trim()}
