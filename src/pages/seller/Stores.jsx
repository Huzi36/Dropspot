...
export default function SellerStores() {
    ...
    useEffect(() => {
        if (profile) {
            setShopifyUrl(profile.shopify_store_url || '')
            setShopifyToken(profile.shopify_access_token || '')
        }
    }, [profile])
    ...
    async function handleSave() {
        ...
        try {
            const { error } = await supabase.from('profiles')
                .update({ shopify_store_url: shopifyUrl, shopify_access_token: shopifyToken })
                .eq('id', profile?.id)
    ...
    async function handleDisconnect() {
        ...
        await supabase.from('profiles').update({ shopify_store_url: null, shopify_access_token: null }).eq('id', profile?.id)
        setShopifyUrl('')
        setShopifyToken('')
    }

    const isConnected = profile?.shopify_store_url && profile?.shopify_access_token
        ...
        <div className="min-h-screen bg-[#F7F8FA] flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <main className={`flex-1 ${sidebarOpen ? 'ml-64' : 'ml-20'} transition-all duration-300 p-8`}>
                <div className="mb-8">
                    ...
                    <p className="text-gray-500 mt-1">Connect your Shopify store to automatically import orders.</p>
                </div>
                ...
                <div className={`rounded-2xl p-6 mb-6 flex items-center gap-4 ${isConnected ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
                    ...
                    <div>
                        ...
                        <p className={`text-sm ${isConnected ? 'text-green-600' : 'text-yellow-600'}`}>
                            {isConnected ? shopifyUrl : 'Connect your Shopify store to start importing orders automatically'}
                        </p>
                    </div>
                    {isConnected && (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            { step: '1', title: 'Connect Store', desc: 'Enter your Shopify store URL and access token', icon: '🔗' },
                            { step: '2', title: 'Customer Orders', desc: 'We pull orders from your Shopify store automatically', icon: '📥' },
                            { step: '3', title: 'Pay & Fulfill', desc: 'Pay on Dropspot and we route to supplier for shipping', icon: '🚚' },
                        ].map(item => (
                            <div key={item.step} className="flex gap-4 p-4 bg-[#F7F8FA] rounded-xl">
                                <div className="w-10 h-10 bg-[#143D59] rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0">{item.step}</div>
                                <div>
